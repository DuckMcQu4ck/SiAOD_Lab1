...
{
    int* newArray = CopyArray(array, size);
...
float* BubbleSort(float* array, size_t size)
{
    float* newArray = CopyArray(array, size);
    for (size_t j = 1; j < size; ++j)
        for (size_t i = 0; i < size - j; ++i)
        {
            if (newArray[i + 1] < newArray[i])
...
int* ShakerSort(int* array, size_t size)
{
    int* newArray = CopyArray(array, size);
    int control = size - 1;
    int left = 0;
...
float* ShakerSort(float* array, size_t size)
{
    float* newArray = CopyArray(array, size);
    int control = size - 1;
    int left = 0;
...
size_t ElementValuesCount(int* array, size_t size, function<bool(int)>& predicate)
{
    size_t count = 0;
    for (size_t i = 0; i < size; ++i)
        if (predicate(array[i]))
...
size_t ElementValuesCount(float* array, size_t size, function<bool(float)>& predicate)
{
    size_t count = 0;
    for (size_t i = 0; i < size; ++i)
        if (predicate(array[i]))
